...
# 텍스트 추출 및 저장 함수
def extract_text_from_pdf_excluding_pages_and_ranges(pdf_path, txt_output_path, exclude_pages, exclude_ranges):
    text = ""
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for i, page in enumerate(pdf.pages):
            if i + 1 in excluded_pages:  # 제외할 페이지에 포함된 경우
                continue
...
exclude_ranges = [(25,90)]  

# 텍스트 추출 및 파일 저장 실행
extracted_text = extract_text_from_pdf_excluding_pages_and_ranges(pdf_path, txt_output_path, exclude_pages, exclude_ranges)
